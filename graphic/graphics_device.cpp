	...
	namespace graphic
	{
		...
		GraphicDevice GraphicDeviceManager::create(Window window, BackBufferSize size, bool bWindowed, bool bVSync)
		{
			...
			try
			{
				...
				device->param->Init(window, (int)back_buf_size.x, (int)back_buf_size.y, bWindowed, bVSync);
			}
			catch(...)
			{
				return NULL;
		...
		void GraphicDeviceManager::Param::Init(Window window, int WindowWidth, int WindowHeight, bool bWindowed, bool bVSync)
		{
			if(this->initialized)
				return;

			this->size.set(WindowWidth, WindowHeight);

			IDirect3DDevice9 *device = NULL;
			try
			{
				//Direct3Dの初期化
				if(!(this->direct3D9 = Direct3DCreate9(D3D_SDK_VERSION)))
					throw std::runtime_error("Direct3D9の初期化に失敗");

				//ディスプレイモードの取得
				D3DDISPLAYMODE d3ddm;
				if(FAILED(this->direct3D9->GetAdapterDisplayMode(D3DADAPTER_DEFAULT, &d3ddm)))
				...
				//デバイスの作成
				if(FAILED(this->direct3D9->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL,
					...
					if(FAILED(this->direct3D9->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL,
						...
						if(FAILED(this->direct3D9->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_REF,
							(HWND)window->getHandle(), D3DCREATE_HARDWARE_VERTEXPROCESSING, &d3dpp, &device)))
							if(FAILED(this->direct3D9->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_REF,
								(HWND)window->getHandle(), D3DCREATE_SOFTWARE_VERTEXPROCESSING, &d3dpp, &device)))
								throw std::runtime_error("Direct3D9の初期化に失敗");
			...
			this->direct3D9Device = device;

			// バックバッファへの参照の獲得
			IDirect3DSurface9* back =NULL;
			if(FAILED(this->direct3D9Device->GetRenderTarget(0, &back))){
